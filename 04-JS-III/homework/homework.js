...
function promedioResultadosTest(resultadosTest) {
  ...
  for (var x = 0; x < resultadosTest.length; x++){
    test += resultadosTest[x];
  }
  return test / resultadosTest.length;
}
...
function numeroMasGrande(numeros) {
  ...
  var numerMayor = 0;
  for (var x = 0; x < numeros.length; x++){
    if (numeros[x] > numerMayor){
      numerMayor = numeros[x];
    }
  }
  return numerMayor;
}
...
  // Si no se pasan argumentos devuelve 0. Si se pasa un argumento, simplemente devuélvelo
  ...
  if (arguments.length === 0){
    ...
    for (var x = 0; x < arguments.length; x++){
      acumulador = acumulador * arguments[x];
    }
  }
  return acumulador;
}
...
function breakStatement(numero) {
  ...
  for (var x = 0; x < 10; x++){
    ...
    if (suma === x){
      ...
      arrayNnumeros.push(suma);
    }
  }
  if (arrayNnumeros.length < 10){
    return 'Se interrumpió la ejecución';
  }
  else if (arrayNnumeros.length === 10){
    return arrayNnumeros;
  }
}
